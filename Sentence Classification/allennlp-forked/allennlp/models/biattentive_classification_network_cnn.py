...
class BiattentiveClassificationNetwork(Model):
    ...
    def __init__(self,
        ...
        self._pos_embedding = nn.Embedding(500, 300).to("cuda:0")
        self._scale = (torch.sqrt(torch.FloatTensor([0.5]))).to("cuda:0")
        self._emb2hid = (nn.Linear(300, 300)).to("cuda:0")
        self._hid2emb = (nn.Linear(300, 300)).to("cuda:0")
        self._convs = nn.ModuleList([(nn.Conv1d(in_channels = 300,out_channels = 600,kernel_size = 3,padding = 1)).to("cuda:0") for _ in range(10)])
        self._do = (nn.Dropout(0.3)).to("cuda:0")
    ...
    def forward(self,  # type: ignore
        ...
        pos = torch.arange(0, pre_encoded_text.shape[1]).unsqueeze(0).repeat(pre_encoded_text.shape[0], 1)
        pos = pos.to("cuda:0")
        pos_embedded = self._pos_embedding(pos)
        pos_embedded = pos_embedded.to("cuda:0")
        edd = self._do(pre_encoded_text + pos_embedded)
        edd = edd.to("cuda:0")
        conv_input = self._emb2hid(edd)
        conv_input = conv_input.to("cuda:0")
        conv_input = conv_input.permute(0, 2, 1)
        conv_input = conv_input.to("cuda:0")

        for i, conv in enumerate(self._convs):

            conved = conv(self._do(conv_input))
            conved = conved.to("cuda:0")
        ...
        conved = conved.to("cuda:0")
        combined = (conved + edd) * self._scale
        combined = combined.to("cuda:0")
        encoded_tokens = torch.cat((combined,conved),2)
        encoded_tokens = encoded_tokens.to("cuda:0")
